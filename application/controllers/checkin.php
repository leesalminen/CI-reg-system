...
';
	...
	if($_POST['datepicker']) { $date = $_POST['datepicker']; //$date = date('Y-m-d',strtotime($date)); //echo $date; 
	} else { die; }
		
	$query = $this->db->query('SELECT class_titles.classname,class_schedule.id as classid FROM class_schedule LEFT JOIN class_titles as class_titles ON class_titles.id = class_schedule.classtitleid
 WHERE startdate LIKE "' .$date. '%" AND cancelled = "No"');
...
//		$output = '<table>';
		...
		$sql = "SELECT student.firstname,student.lastname,student.email,company.companyname,class_titles.classname,class_schedule.instructor,class_schedule.location,class_schedule.duration,class_schedule.cancelled FROM enrollment
JOIN company on company.id = enrollment.companyid
JOIN student on student.id = enrollment.studentid
JOIN class_titles on class_titles.id = enrollment.classid
JOIN class_schedule on class_schedule.id = enrollment.datesid
WHERE `startdate` LIKE '" .$date. "%' AND `datesid` = '" .$class. "'  ";
		$query = $this->db->query($sql);
$numRows = $query->num_rows();
$row2 = $query->row();
		...
		if ($query->num_rows() > 0)
			{
	   			...
	   			if ( ! write_file('.' .$filename, $table))
				{
     				...
     				echo json_encode('<h2 style="color:red;"><a href="' .$filename. '" target="_blank">Click Here to Download Sign In Sheet</a></h2>');
				}
